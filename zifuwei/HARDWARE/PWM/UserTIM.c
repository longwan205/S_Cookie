...
void TIM8_Configuration(void)//TIM8四通道分别输出PWM1/PWM2/PWM3/PWM4,对应引脚为PC6/PC7/PC8/PC9
{
	...
	TIM_TimeBaseStructure.TIM_CounterMode = TIM_CounterMode_Up;
	TIM_TimeBaseStructure.TIM_Period = PWMPeriod;//。PWM频率=72000000/7200=10Khz
	TIM_TimeBaseStructure.TIM_ClockDivision = 0;
	TIM_TimeBaseStructure.TIM_RepetitionCounter = 0;
	TIM_TimeBaseInit(TIM8, &TIM_TimeBaseStructure);
	
	TIM_OCInitStructure.TIM_OCMode = TIM_OCMode_PWM1;//先高后低
	TIM_OCInitStructure.TIM_OutputState = TIM_OutputState_Enable;
	TIM_OCInitStructure.TIM_OutputNState = TIM_OutputNState_Disable;
...
{		/*
   ...
   //s32 MotorSpeed,PwmOne_Set,PwmOne_Mode;
	...
	//MotorSpeed = control;//读取PID的输出

	if(out_new > 1200)  out_new =   1200-1 ;//上限 CCR的值必须小于或等于ARR的值
	if(out_new <-1200)  out_new = -(1200-1);//下限 
...
/*	
	...
	if(PwmOne_Mode) {TIM1->CCR1 = 7200; TIM1->CCR2 = 7200-PwmOne_Set;} //MotorForward
	else { TIM1->CCR1 = 7200-PwmOne_Set; TIM1->CCR2 = 7200;} //MotorBack
